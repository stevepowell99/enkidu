...
export async function apiHandleRequest({ method, pathname, searchParams, headers, bodyText }) {
  ...
  if (m === "POST" && p === "/api/spaced-rep/next") {
    const data = String(headers?.["content-type"] || "").includes("application/json")
      ? safeJsonParse(bodyText) || {}
      : parseFormUrlEncoded(bodyText);
    const model = String(data.model || "").trim();
    ...
    for (const e of entries) {
      ...
      let srWeight = 0;
      for (const t of tags) {
        const m = String(t || "").match(/^spaced-rep(?::\s*(\d+))?$/i);
        if (m) {
          srWeight = m[1] ? Number(m[1]) : 1;
          break;
        }
      }
      ...
      const fm = parseFrontMatter(await readFileUtf8(path.join(REPO_ROOT, e.path)));
      const currentPriority = Number(fm["spaced-rep-priority"]) || 3;
      
      // Weight = spaced-rep relevance * current priority.
      const weight = srWeight * currentPriority;
      srCandidates.push({ entry: e, weight, currentPriority });
    }
    
    if (!srCandidates.length) {
      return { statusCode: 200, json: { question: null, memoryId: null } };
    }
    
    // Weighted random selection (higher priority = more likely).
    const totalWeight = srCandidates.reduce((sum, c) => sum + c.weight, 0);
    let rand = Math.random() * totalWeight;
    let selected = srCandidates[0];
    ...
    // Generate question from memory content.
    const memPath = path.join(REPO_ROOT, selected.entry.path);
    const memContent = await readFileUtf8(memPath);
    const stripped = stripFrontMatter(memContent).trim();
    ...
    const messages = [
      ...
      {
        role: "user",
        content: `Memory:\n\n${stripped.slice(0, 2000)}\n\nGenerate ONE test question (with answer options if applicable).`,
      },
    ];
    
    const question = await openaiChat(messages, { model: model || "gpt-4o-mini" });
    
    return {
      statusCode: 200,
      json: {
        question,
        memoryId: selected.entry.id || selected.entry.path,
        memoryPath: selected.entry.path,
        currentPriority: selected.currentPriority,
      },
    };
  }

  if (m === "POST" && p === "/api/spaced-rep/submit") {
    const data = String(headers?.["content-type"] || "").includes("application/json")
      ? safeJsonParse(bodyText) || {}
      : parseFormUrlEncoded(bodyText);
    const memoryId = String(data.memoryId || "").trim();
    const rating = Math.max(1, Math.min(5, Number(data.rating) || 1));
    
    if (!memoryId) return { statusCode: 400, json: { error: "Missing memoryId" } };
    
    // Find memory by ID or path.
    const idx = await loadIndex();
    const entry = (idx.entries || []).find(e => e.id === memoryId || e.path === memoryId);
    if (!entry) return { statusCode: 404, json: { error: "Memory not found" } };
    
    const memPath = path.join(REPO_ROOT, entry.path);
    assertWithinMemories(memPath);
    let md = await readFileUtf8(memPath);
    const fm = parseFrontMatter(md);
    const oldPriority = Number(fm["spaced-rep-priority"]) || 3;
    
    // Adjust priority: 1-2 (wrong) → +1, 3 (ok) → no change, 4-5 (good) → -1.
    let newPriority = oldPriority;
    if (rating <= 2) newPriority = Math.min(5, oldPriority + 1);
    else if (rating >= 4) newPriority = Math.max(1, oldPriority - 1);
    ...
    if (fmEnd > 0) {
      ...
      for (let i = 1; i < fmEnd; i++) {
        if (lines[i].startsWith("spaced-rep-priority:")) {
          lines[i] = `spaced-rep-priority: ${newPriority}`;
          found = true;
          break;
        }
      }
      if (!found) {
        lines.splice(fmEnd, 0, `spaced-rep-priority: ${newPriority}`);
      }
    }
    
    md = lines.join("\n");
    await writeFileUtf8(memPath, md);
    
    // Refresh index to pick up changes.
    const newEntries = await buildIndex();
    await writeIndex(newEntries);
    
    return {
      statusCode: 200,
      json: { ok: true, oldPriority, newPriority, memoryPath: entry.path },
    };
  }
